...
- **Карусели** — визуальный генератор каруселей со слайдером-пагинатором, раздельным скачиванием текущего или всех слайдов в PNG, и встроенным публикатором. Поддерживает 4 дизайна (Journal, Notes, Notes Dark, Influencer).
- **Медиабиблиотека**, кросс-публикация между аккаунтами, активити-лог
- **Премиальный UX диалогов** — замена стандартных браузерных окон `confirm()` на кастомные React-модали с размытием фона (`backdrop-blur-sm`) для подтверждения удаления шаблонов, папок, ручной публикации и отмены пакетов.
- **Индикаторы и защита от сбоев** — полноэкранный локер при импорте больших пакетов шаблонов, предотвращающий случайные дублирования запросов.
- **Бесшовная навигация** — панель настроек в левом сайдбаре с быстрым возвратом к профилю и оптимизированным фоновым обновлением данных без мерцания интерфейса.
